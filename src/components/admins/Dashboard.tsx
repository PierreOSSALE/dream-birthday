...
function Stat({ label, value, accent }: { label: string; value: number | string; accent?: boolean }) {
  return (
    <HoverScale>
      <div className={`rounded-2xl shadow-card p-6 ${accent ? "gradient-gold text-white" : "bg-white"}`}>
...
export function Dashboard() {
  ...
  return (
    <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <Stat label="Presents" value={`${confirmed} (${pct}%)`} accent />
      <Stat label="Absents" value={declined} />
